 ...
 */
@Entity
@Table(name="PASES", uniqueConstraints = {@UniqueConstraint(columnNames = "codsala"), @UniqueConstraint(columnNames = "codpelicula")})
@NamedQuery(name="Pase.findAll", query="SELECT p FROM Pase p")
public class Pase implements Serializable {
	private static final long serialVersionUID = 1L;

	@Id
	@Column(name = "codpase", nullable = false, unique = false)
	private int codpase;
	...
	public String toString() {
		return "Pase [codpase=" + codpase + ", fechaPase=" + fechaPase + ", tipoPase=" + tipoPase + ", sala=" + sala
				+ "]";
	}
